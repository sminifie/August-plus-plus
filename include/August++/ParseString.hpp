...
namespace August
{
	...
	class ParseString : public StringView
	{
		// We never copy or move
		void operator=(const ParseString&) = delete;
		void operator=(const ParseString&&) = delete;
		ParseString() = delete;
	...
	public:
		constexpr ParseString(const ParseString& rhs) noexcept : StringView(rhs)
		{
		}
		constexpr ParseString(const StringView::const_pointer base, const StringView::size_type count) noexcept : StringView(base, count)
		{
		}
		...
		static ParseToken Parse(ParseIterator& start)
		{
			...
			return ParseToken(stringView.data(), stringView.size());
		}
