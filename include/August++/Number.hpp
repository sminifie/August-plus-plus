...
namespace August
{
	...
	public:
		...
		static Token Parse(Character character, ParseIterator& start)
		{
			...
			if (!IsDigit(character))
			{
				...
				throw ParseException(iterator, "Number expected");
			}
			...
			if (character != '0')
			{
				...
				for (;;)
				{
					...
					if (!*iterator)
					{
						...
						start = iterator;
						return Token(negative ? -integerValue : integerValue);
					}
					character = *iterator;
					if (!IsDigit(character))
					{
			...
			if (character == '.')
			{
				...
				{
					// Number and decimal point followed by EOF or non-digit
				...
				for (;;)
				{
					...
					if (!*iterator)
					{
						...
						start = iterator;
						return Token((negative ? -floatingPointValue : floatingPointValue) / PowerOf10<double>::For(static_cast<std::size_t>(iterator - fractionalDigitsStart)));
					}
					character = *iterator;
					if (!IsDigit(character))
					{
			...
			for (;;)
			{
				...
				if (!*iterator || !IsDigit(*iterator))
				{
					...
					if (exponentValue < 0)
						...
						floatingPointValue = floatingPointValue * PowerOf10<double>::For(static_cast<std::size_t>(exponentValue));
					return Token(negative ? -floatingPointValue : floatingPointValue);
				}
				character = *iterator;
			}
		}
